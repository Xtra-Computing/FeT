...
class LearnableFourierPositionalEncoding(nn.Module):
    def __init__(self, G: int, M: int, F_dim: int, H_dim: int, D: int, gamma: float):
        ...
        # If D is not divisible by G, we pad D to be divisible by G
        if self.D % self.G != 0:
            self.pad = self.D % self.G
            self.D = self.D // self.G * self.G
        else:
            self.pad = 0
    ...
    def forward(self, x):
        ...
        if self.pad != 0:
            PEx = torch.cat([PEx, torch.zeros((N, self.pad)).to(x.device)], dim=-1)
        return PEx
